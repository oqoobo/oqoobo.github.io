...
import {  addDescriptionListener, addCommentListener as addCommentListeners} from "./Gamepage.js"
import { displayCollection, usersCollection, clearCollection } from "./Collection.js";
import {createTimeline} from "./Activity.js";
import { emptyFriendlist } from "./Friendlist.js";
import { getNames } from "./GameAdding.js";
import { gameNameList } from "./initDataEntry.js";
import { usersLastRating, checkMyCollection } from "./Homepage.js";

//Wird von Activity, Collection, Game, GameAdding, initDataEntry, und Profile verwendet.
/*INfo zu Reviews: 
- Kommentare sind beim jeweiligen Game gespeichert.
g1u1c1
date: "2020-3-16"
picture: "hexCode" << maxSize: 256 MB
reviewID: "g1u1r"
text: "its Good"
time: "15:43:42"
userName: "carl"

- Ratings sind beim jeweiligen Game. sie geben einen Score an. es kann max 1 Rating pro User geben.
die Rating ID (RatingList) ist im Kommentar die reviewID. 
Der erste Kommentar des users mit der userID in der ID des Ratings zu einen Rating ist das Review. darauffolgenge Kommentare zu der RatingID sind Kommentare zum review,.
...
Aufbau der Betietelung:
IndexNumer Name
    Input: Zu benutzende Inputvariablen in der richtigen reinfolge.
    Output: Die wahrscheinlich zu benutzenen Ausgabevariablen.
    Auskommentierte Aufrufbefehl mit beispiel variablen.
Index:
...
Todo:
commentare zu reviewsLinken
bilderFunktion für kommentare.
gameID von "1" zu "g1" und das gleiche für user
Timline ist deine Aktivitäten und freunde gehen freundesliste durch und greifen auf deine Timeline zu.
getRatingID
gamePagecomments mit rating ref
gamePage commentar zu comment steht zweimal text da.
GamePage alle ratings
Effizenter
discription ändern
Suchen nach spielen. Optional
Bool ob freunden anzeigen?
accound löschen?
*/
...
*/
  ...
  ref.once("value", function(snapshot) {
    var temp = snapshot.child("user").child(friendsName).child("friendsList").val();
    let json = JSON.parse(JSON.stringify(temp)), friends = Object.keys(json);
...
let userRef = ref.child("user").child(addFriendUserName).child("friendsList").child(friendName);

      //Check if friend exists.
        ref.once("value", function(snapshot) {
          var temp = snapshot.child("user").val();
          let nameJson = JSON.parse(JSON.stringify(temp)), names = Object.keys(nameJson);
          if(names.includes(friendName)){
            if(snapshot.child("user").child(addFriendUserName).child("friendsList").child(friendName).exists()){
            }else{
            userRef.set(true);
            }
          }
...
*/
  function deletFriend(friend,deletFriendUserName){
    let userFriendlistRef = ref.child("user").child(deletFriendUserName).child("friendsList").child(friend);
...
function makeComments(gameName,user,text,reviewLink,pictureText){
  let comments = [],userID, gameID, commentNum = 0,today = new Date(), commentDate = today.getFullYear()+"-"+(today.getMonth()+1)+"-"+today.getDate(),commentTime = today.getHours() + ":" + today.getMinutes() + ":" + today.getSeconds(),commentName=user, commentText= text;
  ref.once("value", function(snapshot) {
  var temp = snapshot.child("game").child(gameName).child("commentList").val();
  ...
  commentNum++;
  commitComment(gameName,gameID,userID,commentNum,commentDate,commentText,commentTime,commentName,reviewLink,pictureText);
  let commentRef = ref.child("game").child(gameName).child("commentList").child("g"+gameID+"u"+userID+"c"+commentNum),
  commentFriendsJson = snapshot.child("user").child(commentName).child("friendsList").val(),
  commentJson = JSON.parse(JSON.stringify(commentFriendsJson)), commentFriends = Object.keys(commentJson),commentTag = "g"+gameID+"u"+userID+"c"+commentNum, 
  commentFill={name: commentName,
    text:commentText, date: commentDate, time: commentTime, game:gameName, ratingRef: reviewLink, picture: pictureText};
...
function commitComment(gameName,gameID,userID,commentNum,commentDate,commentText,commentTime,commentName,commentReviewLink,commentPictureText){
  let commentRef = ref.child("game").child(gameName).child("commentList").child("g"+gameID+"u"+userID+"c"+commentNum), 
  commentUserRef = ref.child("user").child(commentName).child("commentList").child("g"+gameID+"u"+userID+"c"+commentNum);
...
function timeLine(timeLineUserName,timeLineTag,timeLineFill){
  let timeLinefriendRef = ref.child("user").child(timeLineUserName).child("timeline").child(timeLineTag);
...
function setRating(ratingGame,ratingUser,ratingRating){
  ref.once("value", function(snapshot) {
  let ratingUserID = snapshot.child("user").child(ratingUser).child("userID").val(),
  ratingGameID = snapshot.child("game").child(ratingGame).child("gameID").val(),
  ratingRef = ref.child("game").child(ratingGame).child("ratingList").child("g"+ratingGameID+"u"+ratingUserID+"r"),
  ratingUserRef = ref.child("user").child(ratingUser).child("gameList").child(ratingGame),ratingUserRefVal= "g"+ratingGameID+"u"+ratingUserID+"r",
  ratingTimeLineTag = "g"+ratingGameID+"u"+ratingUserID+"r",
  today = new Date(), ratingDate = today.getFullYear()+"-"+(today.getMonth()+1)+"-"+today.getDate(),ratingTime = today.getHours() + ":" + today.getMinutes() + ":" + today.getSeconds(),
  ratingTimeLineFill = {name: ratingUser, game: ratingGame, date: ratingDate, time: ratingTime, rating: ratingRating};
  ratingUserRef.set(ratingUserRefVal);
  timeLine(ratingUser,ratingTimeLineTag,ratingTimeLineFill);
  ratingRef.set(ratingRating);
}, function (error) {
    console.log("Error: " + error.code);
 });
...
function getRating(ratingGame,ratingUser){
  ref.once("value", function(snapshot) {
    let ratingUserID = snapshot.child("user").child(ratingUser).child("userID").val(),
    ratingGameID = snapshot.child("game").child(ratingGame).child("gameID").val(),
    ratingResult = snapshot.child("game").child(ratingGame).child("ratingList").child("g"+ratingGameID+"u"+ratingUserID+"r").val();
  }, function (error) {
      console.log("Error: " + error.code);
   });
}
...
function getCollection(collectionUserName, num){
  ...
  ref.once("value", function(snapshot) {
      let collectionInput = snapshot.child("user").child(collectionUserName).child("gameList").val(),
      ...
      collectionGames.forEach(elementGame => {
        collectionGamesRatingRef= snapshot.child("user").child(collectionUserName).child("gameList").child(elementGame).val(); 
        collectionGamesRating = snapshot.child("game").child(elementGame).child("ratingList").child(collectionGamesRatingRef).val();
        collectionGamesTotalRating = snapshot.child("game").child(elementGame).child("rating").val();
        collectionGamesGenre = snapshot.child("game").child(elementGame).child("genre").val();
        collectionGamesPublisher = snapshot.child("game").child(elementGame).child("publisher").val();
        collectionRatings.push([elementGame,collectionGamesRating,collectionGamesTotalRating,collectionGamesGenre,collectionGamesPublisher]);
  // Extra methode die pro game ein div container an das collection div hängt. Übergeben wird name, publisher, genre, rating
     
      if ( num === 0)
       {displayCollection(collectionUserName, elementGame, collectionGamesPublisher,collectionGamesGenre,collectionGamesTotalRating);}
...
*/
  function newGame(newGameName,newGameDiscription,newGameGenre,newGamePublished,newGamePublisher,newGameDeveloper,newGameOS, id){
    ref.once("value", function(snapshot) {
      ...
      ref.child("game").child(newGameName).set({
       ...
       "ratingList" : {[newGameRatingFill] : 0},
...
function login(loginName,loginPassword){
  let count = 0;
  ref.once("value", function(snapshot) {
  if(snapshot.child("user").child(loginName).child("password").val() === loginPassword ){
...
function gamePage(gamePageGameName,gamePageUserID){
  ...
  gamePageDiscription,gamePageGenre,gamePagePublished, gamePageRating, gamePageMyRating, gamePageComments = [], gamePageCommentKeys, gamePageDeveloper,gamePagePubisher,gamePageOs,gamePagePicture;
  ...
  gamePageRef.once("value", function(snapshot) {
    ...
    gamePageRatingKeyArray = Object.keys(gamePageRatingJson),gamePageRatingAgregat=[], gamePageGameID, gamePageAllRatings = [];
    gamePageRatingKeyArray.forEach(gamePageRatingElement => {  
    let tempNum = snapshot.child("ratingList").child(gamePageRatingElement).val();
    ...
    gamePageMyRating = snapshot.child("ratingList").child("g"+gamePageGameID+"u"+gamePageUserID+"r").val();
    //get comments
    let gamePageCommentKeysList = snapshot.child("commentList").val(),
    gamePageCommentgJson = JSON.parse(JSON.stringify(gamePageCommentKeysList)), gamePageCommentTrashArray; 
    gamePageCommentKeys = Object.keys(gamePageCommentgJson)
    gamePageCommentKeys.forEach(gamePageCommentElement => {
      let gamePageCommentDate = snapshot.child("commentList").child(gamePageCommentElement).child("date").val(), 
      gamePageCommentText = snapshot.child("commentList").child(gamePageCommentElement).child("text").val(), 
      gamePageCommentTime = snapshot.child("commentList").child(gamePageCommentElement).child("time").val(), 
      gamePageCommentUser = snapshot.child("commentList").child(gamePageCommentElement).child("userName").val(),
      gamePageCommentReviewLink = snapshot.child("commentList").child(gamePageCommentElement).child("reviewLink").val(),
      gamePageCommentPicture = snapshot.child("commentList").child(gamePageCommentElement).child("picture").val();
      gamePageComments.push([gamePageCommentElement,gamePageCommentDate,gamePageCommentText,gamePageCommentTime,gamePageCommentUser,gamePageCommentReviewLink,gamePageCommentPicture]);
    });
    ...
    //console.log("overall Rating:" + gamePageRating);
    gameInfoBox[15].textContent = "Average Rating: " + gamePageRating + "/10";
    //console.log("My Rating: "+ gamePageMyRating);
    if(gamePageMyRating){
      gameInfoBox[17].textContent = " My Rating: " + gamePageMyRating + "/10";
    }
    else{
    ...
    //Picture  
      if(!(element[6]==="") && element[6]){
         ...
         //This is a workaround to avoid such broken pictures
          let picStr = element[6] + "";
          let httpStr = "http://127";
          if(picStr.indexOf(httpStr) === -1){
      ...
      addCommentListeners(reviewContent[9], reviewContent[11], element[0], review);
...
*/
  ...
  function getGameNames(){
    ref.once("value", function(snapshot) {
      ...
      let json = JSON.parse(JSON.stringify(temp)), gameNames = Object.keys(json);
      //hier weiterarbeiten
      getNames(gameNames);
      gameNameList(gameNames);
    }, function (error) {
      console.log("Error: " + error.code);
      return null;
   });
...
function getTimeLine(getTimeLineUser, num){
  ...
  let timeLineArray = [];
    ...
    switch (num) {
      case 1:   
        ref.once("value", function(snapshot) {
          var temp = snapshot.child("user").child(getTimeLineUser).child("timeline").val();
          let json, timeline;
          if(temp != null){json = JSON.parse(JSON.stringify(temp)), timeline = Object.keys(json);
            timeLineArray.push(json);
          }
          usersLastRating(timeLineArray);
          console.log("single timeline");
        }, function (error) {
          console.log("Error: " + error.code);
          return null;
        });
      ...
      default: 
        ref.once("value", function(snapshot) {
          var temp = snapshot.child("user").child(getTimeLineUser).child("friendsList").val();
          let json = JSON.parse(JSON.stringify(temp)), friends = Object.keys(json);
          friends.forEach(timeLineElement => {
            var timeLineTemp = snapshot.child("user").child(timeLineElement).child("timeline").val();
            let timeLineJson , timeLineOneUser ;
            if(timeLineTemp != null){timeLineJson = JSON.parse(JSON.stringify(timeLineTemp)), timeLineOneUser = Object.keys(timeLineJson);
        ...
        if ( num === 2)
        {
          createRecentActivity(timeLineArray);
            console.log("special Timeline");
        }
        else {
          createTimeline(timeLineArray);
          console.log("normal timeline")
        }
...
function changeDescription(cDGameName,cDDescriptionText){
  ref.child("game").child(cDGameName).child("discription").set(cDDescriptionText);
}
...
function updateRating(uRGameName){
  ...
  uRRef.once("value", function(snapshot) {
    ...
    uRRatingKeyArray = Object.keys(uRRatingJson),uRRatingAgregat=[], uRGameID, uRAllRatings = [];
    uRRatingKeyArray.forEach(uRRatingElement => {  
    let tempNum = snapshot.child("ratingList").child(uRRatingElement).val();
    uRAllRatings.push([uRRatingElement,tempNum]);
    if(tempNum > 0)
    {uRRatingAgregat.push(tempNum);}
    });
    if(uRRatingAgregat !== 0){
      uRRating = uRRatingAgregat.reduce(function(sum, a,i,ar) { sum += a;  return i==ar.length-1?(ar.length==0?0:sum/ar.length):sum},0);
    }else {uRRating = 0}
...
export {newGame, addFriend, login, createNewAccount, getFriendslist, deletFriend, getCollection, loadGamePage, getTimeLine,  makeComments, changeDescription, setRating, getGameNames}